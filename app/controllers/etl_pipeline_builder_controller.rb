...
class EtlPipelineBuilderController < ApplicationController
  ...
  def show
    @executions = @pipeline.pipeline_executions
                          .includes(:user)
                          .order(created_at: :desc)
                          .limit(10)
  ...
  def execute
    ...
    redirect_to pipeline_execution_path(execution),
  ...
  def available_extractors
    ...
    when "database"
      DatabaseExtractor::SUPPORTED_DATABASES
    when "api"
      [ "rest", "graphql", "soap" ]
    when "cloud_storage"
      CloudStorageExtractor::SUPPORTED_PROVIDERS
    when "streaming"
  ...
  def available_warehouses
    WarehouseLoaderService::SUPPORTED_WAREHOUSES.select do |warehouse|
      current_organization.has_warehouse_configured?(warehouse)
    end
  end
  ...
  def recent_pipeline_executions
    PipelineExecution.where(organization: current_organization)
                     .includes(:pipeline_configuration, :user)
                     .order(created_at: :desc)
                     .limit(5)
  end

  def calculate_pipeline_metrics
    executions = @pipeline.pipeline_executions.recent(30.days)

    {
      ...
      average_rows_processed: executions.successful.average(:rows_processed) || 0,
      last_run: executions.first,
