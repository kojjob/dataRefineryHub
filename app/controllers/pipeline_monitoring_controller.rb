...
class PipelineMonitoringController < ApplicationController
  ...
  def system_health
    @queue_metrics = Solid::Queue::Job.group(:queue_name).count
  ...
  def calculate_pipeline_health
    ...
    
    {
      ...
      queue_depth: Solid::Queue::Job.where(queue_name: 'etl').count,
  ...
  def calculate_execution_metrics(execution)
    ...
    
    {
      ...
      records_per_second: execution.records_processed.to_f / duration,
      stages: {
  ...
  def check_worker_status
    ...
    {
      workers: Solid::Queue::Worker.count,
      active_jobs: Solid::Queue::Job.where(finished_at: nil).count,
      failed_jobs: Solid::Queue::FailedExecution.count
    }
  end
