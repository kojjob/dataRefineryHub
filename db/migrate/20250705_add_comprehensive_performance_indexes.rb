class AddComprehensivePerformanceIndexes < ActiveRecord::Migration[8.0]
  ...
  def change
    ...
    unless index_exists?(:pipeline_configurations, [ :organization_id, :status ])
      add_index :pipeline_configurations, [ :organization_id, :status ],
                algorithm: :concurrently,
                name: 'idx_pipeline_configs_org_status'
    end

    unless index_exists?(:pipeline_configurations, :schedule_config)
      add_index :pipeline_configurations, :schedule_config, using: :gin,
                algorithm: :concurrently,
  ...
  def down
    ...
    remove_index :pipeline_configurations, name: 'idx_pipeline_configs_org_status', if_exists: true
    remove_index :pipeline_configurations, name: 'idx_pipeline_configs_schedule_gin', if_exists: true
